            ...
            {silverTransactions.length === 0 ? (
              ...
              silverTransactions.map(tx => (
                <tr key={tx.id} className="hover:bg-gray-50/50 transition-colors text-xs">
                  ...
                  <td className="px-2 py-2.5 font-bold text-gray-900 max-w-[160px]">
                    <div className="truncate">{tx.merchantNormalized || tx.merchantRaw}</div>
                    <span className="block text-[10px] font-normal text-gray-400 truncate max-w-[140px]" title={tx.emailSubject}>{tx.emailSubject || 'Source Raw Email'}</span>
                  </td>
                  <td className="px-2 py-2.5 text-gray-500 whitespace-nowrap">{tx.transactionDate}</td>
                  <td className="px-2 py-2.5 font-bold text-right text-gray-800 whitespace-nowrap">{tx.amount.toFixed(2)} {tx.currency}</td>
                  <td className="px-2 py-2.5 text-center">
                  ...
                  <td className="px-2 py-2.5 text-center">
                    <button
                      ...
                      className="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold px-2 py-1 rounded-lg uppercase tracking-wider cursor-pointer shadow-sm transition-all duration-150"
                    >
                      Review
                    </button>
                  </td>
