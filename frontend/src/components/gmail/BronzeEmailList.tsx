...
export const BronzeEmailList: React.FC<BronzeEmailListProps> = (props) => {
  ...
  const displayedEmails = searchQuery.trim()
    ? visibleRawEmails.filter((email) => {
  ...
  return (
    <div>
      {/* Sub-tab bar (Transactions / Non-Transactional) */}
      <div className="border-b border-gray-100 bg-gray-50/70 flex flex-col md:flex-row justify-between items-stretch md:items-center px-4 py-2 gap-3">
        ...
        {/* Batch action buttons (only shown when items are checked) */}
      ...
      <div className="p-4 bg-gray-50/20 border-b border-gray-100 space-y-4">
        {/* Row 1: Status Filter + Item Count */}
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex flex-col min-w-[150px]">
            <label
              htmlFor="bronze-status-filter"
              className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5"
            >
              Filter:
            </label>
            <select
          ...
          <div className="flex items-end pb-1">
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">
              {displayedEmails.length} Items
            </span>
          </div>
        </div>

        {/* Row 2: Keyword Search + Date Filters */}
