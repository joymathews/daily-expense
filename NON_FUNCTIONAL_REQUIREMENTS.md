...
- [NFR-LLM-3] Semantic Embedding Retrieval Latency: Generating raw email vector embeddings locally via Ollama and matching them against the correction history using cosine similarity must resolve within 150ms during extraction, falling back instantly (within 50ms) to recency-based selection if the embedding service is unreachable.
...
- [NFR-ANALYSIS-14] Savings Recommendations Performance: The compilation and matching of savings recommendations must complete within 20ms of state changes to ensure immediate rendering on load or slider adjustments.
