...
- [NFR-USAB-7] Soft-Delete and Data Integrity: Reverting and deleting records must never cause database referential integrity failures. Only active Bronze records (raw inputs) can be soft-deleted. Reverting a Silver or Gold transaction must safely hard-delete those records without leaving orphaned records or breaking constraints. The Trash Bin must display only deleted Bronze raw inputs and allow restoring them.
