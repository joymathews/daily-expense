...
- [FUNC-GMAIL-31] Multi-Stage Delete & Trash Bin: The user must be able to initiate a deletion from any Medallion layer exclusively through the details or correction modal. The system must prompt the user with a confirmation dialog to select which stages of the Medallion pipeline (Bronze, Silver, Gold) should be soft-deleted, defaulting to the stage from which the deletion was initiated. The user must be able to access all soft-deleted records in a Trash Bin and restore them to their active states with all original cross-stage linkages intact.
