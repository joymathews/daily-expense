...
- [FUNC-GMAIL-4] Display: The user must be able to scan their staging review queue chronologically in a high-density table that displays the transaction date first, followed by separate merchant and amount columns, with secondary details (Category & Method) and review actions (Status & Action) stacked vertically to ensure a clear visual structure with no horizontal clutter on desktop screens.
