...
- [FUNC-GMAIL-9] Email Detail View: The user must be able to click on the subject field of a fetched raw email in the Bronze table to view its details (sender, subject, date, and full decoded plain-text body content) in a secure modal popup.
...
- [FUNC-GMAIL-16] Date Range Filtering: The user must be able to filter the listed records in all Medallion tables (Bronze raw emails, Silver staging pending, and Gold confirmed ledger) by selecting a start and end date range. The date range filter controls must be rendered contextually in the active view of all three tabs, allowing the user to filter table data dynamically at any stage of the pipeline.
- [FUNC-GMAIL-17] On-Demand Ingestion & Batch Extraction: The user must be able to trigger the Ollama extraction logic manually for a single raw email (from both the checkboxes in the table view and directly within the email detail modal, provided the email is marked as a transaction) or in batches for multiple selected raw emails.
...
- [FUNC-GMAIL-21] Processed Emails Visibility: The user must see a distinct visual indicator (such as a 'Processed' badge) next to raw emails that have already been extracted or approved, and the checkbox for selecting these emails must be disabled to prevent duplicate ingestion or extraction attempts.
...
- [FUNC-GMAIL-31] Pipeline Reversion & Raw Email Deletion: The user must be able to revert records to previous pipeline stages or delete raw emails exclusively through the details or correction modal. Reverting a Gold record (confirmed ledger) completely removes it from the Gold ledger and transitions its parent Silver record (staging) back to pending/error status. Reverting a Silver record completely removes it from Silver staging and marks the corresponding Bronze email as unprocessed. Only active Bronze records (raw emails) can be soft-deleted. The system must prompt the user with a context-sensitive confirmation modal. Users can view all soft-deleted Bronze records in a Trash Bin and restore them to their active states.
