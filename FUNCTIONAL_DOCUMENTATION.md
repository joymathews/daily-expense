...
- [FUNC-GMAIL-4] Display: The user must be able to scan their staging review queue (Silver layer) and verified ledger (Gold layer) chronologically in high-density tables displaying the transaction date first. Category and Method are displayed in separate columns on both tables. Action columns and individual action buttons (such as Review, Correct, or Delete) are removed from the active tables to maximize display space and prevent horizontal clutter, with modal triggers centralized on primary textual identifiers (Subject/Merchant).
...
- [FUNC-GMAIL-31] Multi-Stage Delete & Trash Bin: The user must be able to delete records from any Medallion layer (Bronze, Silver, or Gold) with the deletion action initiated exclusively from the details modal popup, opening a confirmation dialog that prompts them to select the target stages (pre-filled with the source stage). Deleted records must be held in a soft-deleted state, visible in a Trash Bin view, and restorable to their active states with original linkages preserved.
