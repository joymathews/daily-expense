...
- [FUNC-GMAIL-4] Display: The user must be able to scan their staging review queue (Silver layer) and verified ledger (Gold layer) chronologically in high-density tables displaying the transaction date first. For the staging queue, Category and Method are stacked vertically, and Status and Action are stacked vertically. For the verified ledger, Category and Method are stacked vertically, and Action (Correct) is displayed in its own column, preventing horizontal clutter and clipping on desktop screens.
